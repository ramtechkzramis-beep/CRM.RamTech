...
// --- Решение: своя страница на каждую выбранную услугу ---------------------

/**
 * Описание услуги для КП: что это, из чего состоит и что даёт бизнесу.
 * В документ попадают только те блоки, услуги которых клиент реально
 * выбрал — раньше страница «Решение» всегда была одна и рассказывала
 * про AI-ассистента, даже если человек покупал только CRM или сайт.
 */
export type SolutionContent = {
  title: string;
  intro: string;
  areas: InfoCard[];
  /** Каналы подключения — есть у бота и CRM, у сайта не применимо. */
  channels?: string[];
  benefitsTitle: string;
  benefits: string[];
};

const BOT_SOLUTION: SolutionContent = {
  title: "Решение: AI-ассистенты",
  intro:
  ...
  channels: ["WhatsApp", "Telegram", "Сайт / виджет", "Instagram Direct", "Почта", "Телефония"],
...
const CRM_SOLUTION: SolutionContent = {
  title: "Решение: CRM для отдела продаж",
  intro:
    "Заявки и переписка с клиентами обычно оседают в личных мессенджерах менеджеров, а не в компании. CRM RamTech собирает все каналы в одну систему и показывает всю картину продаж в реальном времени.",
  areas: [
    {
      title: "Единая база клиентов",
      text: "Все обращения с мессенджеров, сайта и почты попадают в одну систему: карточка клиента, история переписки и статус сделки видны в любой момент.",
    },
    {
      // Стрелки «→» нет в глифах встроенного шрифта — вместо неё в PDF
      // выводился мусорный символ. Разделяем этапы обычным тире.
      title: "Воронка продаж",
      text: "Заявка — квалификация — предложение — счёт — оплата. Видно, на каком этапе каждая сделка и сколько денег «в пути».",
    },
    {
      title: "База остаётся у компании",
      text: "Контакты и переписка закреплены за компанией, а не за личным телефоном менеджера — увольнение или отпуск сотрудника не означает потерю клиента.",
    },
    {
      title: "Аналитика по менеджерам",
      text: "Конверсия, скорость ответа, число сделок и выручка — по каждому сотруднику и по отделу целиком, в одной панели для руководителя.",
    },
    {
      title: "Повторные продажи",
      text: "Напоминания о повторных заказах, продлениях и сервисном обслуживании — клиент не забывается после первой сделки.",
    },
    {
      title: "Интеграция с AI-ассистентом",
      text: "Если подключён чат-бот, он сам создаёт карточки лидов и сделки из переписки — без ручного переноса данных.",
    },
  ],
  channels: ["WhatsApp", "Telegram", "Сайт / виджет", "Instagram Direct", "Почта", "Телефония"],
  benefitsTitle: "Преимущества для вашего отдела продаж",
  benefits: [
    "Все менеджеры в единой системе, каждый видит свои сделки",
    "Руководитель видит загрузку и результаты отдела онлайн",
    "Клиентская база закреплена за компанией, а не за телефоном",
    "Прозрачная аналитика для планирования и отчётности",
    "Быстрый поиск истории вместо ручного поиска в чатах",
  ],
};

const WEBSITE_SOLUTION: SolutionContent = {
  title: "Решение: сайт для вашего бизнеса",
  intro:
    "Сайт — первая точка контакта с клиентом и канал заявок, который работает круглосуточно. Делаем сайты, которые не просто выглядят современно, а приводят обращения и связаны с остальными вашими системами.",
  areas: [
    {
      title: "Первое впечатление",
      text: "Дизайн под ваш бренд и адаптация под телефоны: большая часть посетителей заходит с мобильных, и сайт должен одинаково работать везде.",
    },
    {
      title: "Заявки и лиды",
      text: "Формы заявки, кнопки мессенджеров и обратного звонка на каждой странице — посетителю всегда есть куда нажать, чтобы оставить контакт.",
    },
    {
      title: "Связь с CRM и ботом",
      text: "Заявки с сайта сразу попадают в CRM или к чат-боту — без ручного переноса и потерянных обращений.",
    },
    {
      title: "Поисковая база",
      text: "Базовая SEO-подготовка: структура, скорость загрузки, мета-теги — сайт находят в поиске, а не только по прямой ссылке.",
    },
    {
      title: "Простое обновление",
      text: "Понятная админ-панель: тексты, фото, цены и услуги меняете сами, без обращения к разработчику.",
    },
  ],
  benefitsTitle: "Преимущества для вашего бизнеса",
  benefits: [
    "Заявки с сайта не теряются — сразу в CRM или менеджеру",
    "Современный вид повышает доверие и конверсию в обращение",
    "Работает на всех устройствах, включая мобильные",
    "Контент можно обновлять самостоятельно",
    "Готов к продвижению в поиске и рекламе",
  ],
};

export const SOLUTION_BY_CATEGORY: Record<ServiceCategory, SolutionContent> = {
  bot: BOT_SOLUTION,
  crm: CRM_SOLUTION,
  website: WEBSITE_SOLUTION,
};
...
export type ProposalServiceItem = {
  ...
  /** Состав из прайса («1 воронка, до 3 пользователей, обучение 1 ч»). */
  composition?: string | null;
  /** Лимит диалогов — есть только у ботов. */
  dialogLimit?: string | null;
};

/** Что входит в конкретную услугу клиента — блок на странице «Предложение». */
export type ProposalServiceDetail = {
  label: string;
  features: string[];
};
...
export type ProposalViewModel = {
  ...
  tierFeatures: string[];
  launchTime: string;
  /** Описания выбранных услуг — по странице на каждую. */
  solutions: SolutionContent[];
  /** Состав по каждой выбранной услуге — из прайса, а не общий список бота. */
  serviceDetails: ProposalServiceDetail[];
...
export function buildProposalViewModel(input: ProposalInput): ProposalViewModel {
  ...
  // Описание идёт только по тем услугам, которые клиент реально выбрал.
  // У клиентов старого формата (пакет без разбивки по услугам) состава нет —
  // им, как и раньше, показываем блок про AI-ассистента.
  const categories =
    input.composition && input.composition.length > 0
      ? [...new Set(input.composition.map((item) => item.category))]
      : (["bot"] as ServiceCategory[]);

  // Состав берём из прайса по каждой услуге отдельно. Общий список
  // PDF_TIER_FEATURES написан под чат-бота — показывать его клиенту,
  // который купил только CRM или сайт, было прямой ошибкой.
  const serviceDetails: ProposalServiceDetail[] = (input.composition ?? [])
    .filter((item) => !!item.composition)
    .map((item) => ({
      label: `${SERVICE_CATEGORY_LABELS[item.category]} · ${PDF_TIER_LABEL[item.package]}`,
      features: [
        ...item
          .composition!.split(",")
          .map((part) => part.trim())
          .filter(Boolean),
        ...(item.dialogLimit ? [item.dialogLimit] : []),
      ],
    }));
  ...
  return {
    ...
    solutions: categories.map((category) => SOLUTION_BY_CATEGORY[category]),
    serviceDetails,
    compositionLabel,
